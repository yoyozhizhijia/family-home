...
const CATEGORIES: Record<string, PortfolioInfo> = {
  ...
  explore: { key: 'explore', title: '探索发现', subtitle: '新奇动植物 · 美丽风景 · 趣味见闻 🌿', emoji: '🌿', gradient: 'from-emerald-400 to-teal-500' },
};
...
export default function PortfolioPage() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(false);
    setMessage(fail === 0 ? `✅ ${success} 张作品上传成功！\n📱 照片墙：https://family-home.onrender.com` : `⚠️ 成功 ${success} 张，失败 ${fail} 张`);
    refresh();
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try { await admin.authedFetch(`/api/photos/${encodeURIComponent(id)}`, { method: 'DELETE' }); refresh(); }
    catch { alert('删除失败'); }
  ...
  return (
    <div>
            ...
            {Object.values(CATEGORIES).map((c) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition ${
                  info.key === c.key ? 'bg-white/25 text-white shadow' : 'bg-white/10 text-white/70 hover:bg-white/20'
                }`}
              >
                {c.emoji} {c.title}
              </button>
      ...
      {message && (
        <div className="max-w-4xl mx-auto mb-4 px-4 py-2 bg-green-100 text-green-700 rounded-lg text-sm text-center">{message}</div>
      )}
      ...
      <div className="max-w-4xl mx-auto">
        {photos.length === 0 && !loading ? (
          <div className="text-center py-20 text-amber-400">
            ...
            <p className="text-sm mt-1">点击「添加作品」上传</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
              {photos.map((photo, index) => (
                <div key={photo.id} className="relative group cursor-pointer rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-shadow fade-in aspect-square bg-amber-100"
                  style={{ animationDelay: `${(index % 8) * 50}ms` }}>
                  <img src={photo.thumbnail_url} alt={info.title} loading="lazy"
                    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                    onClick={() => setLightboxIndex(index)} />
                  {admin.isAdmin && (
                    <button onClick={(e) => { e.stopPropagation(); handleDelete(photo.id); }}
                      className="absolute top-1 right-1 text-xs bg-white/80 text-red-500 rounded-full w-5 h-5 opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
                  )}
                </div>
              ))}
            </div>
            <div ref={loaderRef} className="py-8 text-center">
              ...
              {!hasMore && photos.length > 0 && <p className="text-amber-300 text-sm">— 共 {total} 件作品 —</p>}
            </div>
          </>
        )}
      </div>
      ...
      {lightboxIndex !== null && (
        <Lightbox photos={photos} currentIndex={lightboxIndex} onClose={() => setLightboxIndex(null)} onNavigate={setLightboxIndex} />
      )}
    </div>
  );
}
